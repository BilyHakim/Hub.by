...
const expenseShare = computed(() =>
  data.value.income > 0 ? (data.value.expense / data.value.income) * 100 : 0,
);
const healthyCheckCount = computed(
  () =>
    data.value.financialCheckup.filter((item) => item.status === "healthy")
      .length,
);
...
const trendFormula = (current, previous, trend) =>
  previous === 0
...
const metricComparisons = computed(() => {
  const currentPeriod = periodLabel(
    data.value.periodStart,
    data.value.periodEnd,
    "Periode ini",
  );
  const previousPeriod = periodLabel(
    data.value.previousPeriodStart,
    data.value.previousPeriodEnd,
    "Periode sebelumnya",
  );
  ...
  return {
    income: createPeriodComparison(
      "Perubahan pemasukan",
      data.value.income,
      data.value.previousIncome,
      data.value.incomeTrend,
    ),
    expense: createPeriodComparison(
      "Perubahan pengeluaran",
      data.value.expense,
      data.value.previousExpense,
      data.value.expenseTrend,
      true,
    ),
    savings: createPeriodComparison(
      "Perubahan uang tersisa",
      data.value.savings,
      data.value.previousSavings,
      data.value.savingsTrend,
    ),
    investment: {
  ...
  <section class="page dashboard-page">
    <div class="page-heading">
      ...
      <div class="heading-actions">
        ...
        <RouterLink
          class="secondary-button dashboard-plan-shortcut"
          to="/finance/modules/budget"
          ><ClipboardList :size="18" /> Rencana keuangan</RouterLink
        >
        <RouterLink class="primary-button" to="/finance/transactions"
          ><Plus :size="18" /> Catat transaksi</RouterLink
        >
      </div>
    </div>
    ...
    <div class="dashboard-grid">
      ...
      <article class="panel emergency-card">
        <div class="panel-heading">
          <div>
            <h2>Dana darurat</h2>
            <p>Target 6 bulan pengeluaran</p>
          </div>
        ...
        <div class="progress-track">
          <span
            :style="{ width: `${Math.min(data.emergencyProgress, 100)}%` }"
          />
        </div>
        <div class="progress-caption">
          ...
          <span
            >Kurang
            {{
              currency(Math.max(data.emergencyTarget - data.emergencyFund, 0))
            }}</span
          >
        </div>
        <p class="gentle-note">
